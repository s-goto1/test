...
import test_0613f.work.SearchDao;
import test_0613f.work.WorkDao;

/**
 * Servlet implementation class SearchServlet
 */
@WebServlet("/work/Search")
		...
		if (request.getParameter("idSearch") != null) {
				...
				if (idList.size() > 0 && nameList.size() > 0) {
					...
					Map<String, List<Work>> map = idListLimit.stream()
							.collect(Collectors.toMap(
									s -> s,
									s -> work.findAllByMonthForIdFromAdmin(s, year, month)));
			...
			// 入力フォームに入力あり？
				...
				// レーベンシュタイン距離は適切な長さ？
					...
					// ヒットしたユーザの情報を取得
					List<Work> list = work.findAllByMonthForId(closest, year, month);
		...
		if (request.getParameter("nameSearch") != null) {
			...
			if (name == null || name.equals("")) {
				...
				if (idList.size() > 0 && nameList.size() > 0) {
					...
					// 今月分の規定数の社員の出張精算データを取得
					Map<String, List<Work>> map = nameListLimit.stream()
							.collect(Collectors.toMap(
									s -> s,
									s -> work.findAllByMonthForNameFromAdmin(s, year, month)));
			...
			// 入力フォームに入力あり？
				...
				// レーベンシュタイン距離は適切な長さ？
					...
					// ヒットしたユーザの情報を取得
					List<Work> list = work.findAllByMonthForName(closest, year, month);
